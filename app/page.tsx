...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {page === "profile" && <section className="page-content"><div className="settings-grid"><section className="panel settings"><h2>数据来源</h2><p>先用稳定、透明的方式收集信息。</p><button><Icon name="share"/><span><b>系统分享入口</b><small>手机 App 接入后可用</small></span><em>规划中</em></button><a href="/downloads/TraceNest-Extension-v0.1.0.zip" download><Icon name="book"/><span><b>浏览器扩展</b><small>支持 Chrome 与 Edge，一键保存网页和选中文字</small></span><em className="on">下载</em></a><button><Icon name="inbox"/><span><b>相册截图</b><small>等待 App 授权</small></span><em>规划中</em></button></section><section className="panel settings"><h2>数据空间</h2><p>不同内容使用不同处理方式。</p>{[["普通空间","云同步与 AI 整理","已启用"],["私密空间","仅保存在本地","未启用"],["临时空间","30 天后自动删除","未启用"]].map(([a,b,c]) => <button key={a}><span><b>{a}</b><small>{b}</small></span><em className={c === "已启用" ? "on" : ""}>{c}</em></button>)}</section></div><div className="privacy-note"><b>你的原始内容始终可追溯</b><p>AI 只提出摘要、标签和关联建议，不会自动删除或改写原始内容。你可以随时导出全部数据。</p><button>查看隐私说明</button></div></section>}
    </main>
